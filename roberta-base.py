...
df=pd.read_csv('/workspace/SFLmailCorpus-new4.csv',header=0,sep=',')
# print(df.head())
...
num_of_labels = max(data_labelIndex) + 1
...
class MyNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.bert_0 = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=150).to(device)
...
# 高清图片代码：%config InlineBackend.figure_format = 'svg'
...
#邮件的长度de个数
max([len(data_text[i]) for i in range(len(data_text))])
data_length = [len(data_text[i]) for i in range(len(data_text))]
plt.hist( data_length)


# 设置保存路径
model_dir = "./workspace/Models"
model_name = "bert_base_full_model"
model_path = f"{model_dir}/{model_name}.pt"

# 保存模型
# torch.save(bert_sc.state_dict(), model_path)
111
torch.save(bert_sc, model_path)

# bert_sc = torch.load(model_path)


select = (prediction['送信者の動き(詳細)']=="感謝") & (labels['送信者の動き(詳細)'] == "挨拶")

select_ids = select.loc[select==True]
select_ids = list(select_ids.index.values)

for i in select_ids:
    print(i)
    print(tokenizer.decode(dataset_val[i]['input_ids']))